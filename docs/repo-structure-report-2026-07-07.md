...
1. **Resolve RD-000 before Agent #01 implementation.** `backlog/research-debt.md` already sequences it: update `templates/operating/daily-issue.md`, `article-or-case-study.md` and `social-posts.md` so internal/private experiments can record an intentional publication deferral without appearing incomplete. This is the only open item that blocks the daily rhythm.
2. **Housekeeping pass on programme records.** Link `backlog/candidate-review.md` from `README.md`; reconcile FN-000's status between the Field Notes index and Research Register; replace the all-TBD `REC-001` row; bump the source-of-truth map date and reference this report.
3. **Record the backlog promotion mechanism.** Add a short note to `backlog/master-backlog.md` (or the weekly-review template) stating how provisional candidates from `backlog/candidate-review.md` are promoted — e.g. reviewed at each weekly calibration, promoted in waves of five to ten.
4. **Mark resolved recommendations in the 2026-07-06 report.** One-line resolution notes for its recommendations 2, 3 and 6, matching the treatment its other recommendations already received.
